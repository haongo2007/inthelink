...
// 25.Tooltip
// 26.profile setting form
(function($) {
    "use strict";

    /*=====================
     use for ajax before send
     ==========================*/
    $.ajaxSetup({
        headers: {
...
//grid layout view
    ...
    $('.product-box button .ti-shopping-cart').on('click', function () {
        theme.showNotification('top','right','Item Successfully added to your cart',2);
    });
    ...
    /*=====================
     19.Add to wishlist
     ==========================*/
    $('.ti-heart , .fa-heart, .remove-wish').on('click', function () {
        var pid = $(this).attr('pid');
        if (pid) {
            var url = window.base_url+'influencer/favorite/add';
            ...
            .done(function(data) {
                if (data.state == 'added') {
                    _this.removeClass('ti-heart').addClass('fa fa-heart active');
                }else{
                    if (_this.hasClass('fa')) {
                        _this.removeClass('fa fa-heart active').addClass('ti-heart');
                    }else{
                        _this.parents('tbody').fadeOut('slow');
                    }
                }
                theme.showNotification('top','right',data.message,2);
            })
            .fail(function() {
                theme.showNotification('top','right','Item Errors when added in wishlist',4);
            })
            .always(function() {
...
$('.my-nav li').click(function(event) {
...
$('.link-share').click(function(event) {
    ...
    $.ajax({
        url: window.base_url+'influencer/link/save',
        type: 'POST',
        data: {link: copyText, p_id: pid},
    })
    .done(function(data) {
        if(data == 200){
            theme.showNotification('top','right','Copied to clipboard',2);
        }else{
            theme.showNotification('top','right','Copied to clipboard with error that cannot be saved',3);
        }
    })
});

/*=====================
 27.Products List pipeline sort count ..
 ==========================*/
$('.filtersproducts select').change(function(event) {
    var link = $.trim($(this).attr('path'));
    var to = $(this).val();
