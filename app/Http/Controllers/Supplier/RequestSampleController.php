...
{
    public function index(Requests $requestsample, Request $request)
    {
    	if(Request()->ajax()) {
        ...
        return view('supplier.requestsample.index',['rqsample' => $requestsample->paginate(5)]);
    }
    public function updateStatus($id,$stt)
    {
        ...
        $rq->status = $stt;
        $rq->save();
        return redirect()->route('supplier.requests.sample')->withStatus(__('Request successfully updated.'));
    }
}
